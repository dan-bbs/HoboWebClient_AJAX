...
void AsyncFSWebServer::serverInit() {
    ...
    on("/get", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        json += ", \"temp\":\"" + String(strT)+"\"";
        json += ", \"gpio\":" + String((uint32_t)(((GPI | GPO) & 0xFFFF) | ((GP16I & 0x01) << 16)));
        json += "}";

        AsyncWebServerResponse *response = request->beginResponse(200, "text/html", json);
        response->addHeader("Access-Control-Allow-Origin", "*");
    ...
    //------------------------------------------------------------------------------------------------------
    //write values to controller

    //set heap status, analog input value and all GPIO statuses in one json call

    on("/set", [this](AsyncWebServerRequest *request) {
...
AsyncFSWebServer& AsyncFSWebServer::setSETCallback(SET_CALLBACK_SIGNATURE) {
